...
@app.route('/fb_username')
def render_fb_username():
    return render_template('fb_username.html')
...
def render_match():
    if request.method == "POST":
        ...
        fp_page = False
        fb = get_user(read_data(), users[0])['facebook']
        if fb != '' and '/' not in fb:
            fb_page = 'facebook.com/' + fb
        
        if request.method == "POST":
            return render_template('match.html', match = match, cmap = country_map, movies = movies, full_name = full_name,
                                   pokeim = pokeim, college_im = college_im, band=band, make_band = make_band, fb_page = fb_page)
